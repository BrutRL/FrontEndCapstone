...
export default function UserRoomRequest() {
  ...
  return (
    <Box sx={{ padding: 2 }}>
     <Box  sx={{ marginBottom: 2, display: 'flex', justifyContent: 'space-between', flexDirection: { xs: 'column', sm: 'row' }, alignItems: { xs: 'flex-start', sm: 'center' }, gap: 2,}}>
          <Typography variant="h4" gutterBottom sx={{ fontWeight: 'bold', color: '#374151', letterSpacing: '0.5px', textAlign: { xs: 'center', sm: 'left' }, fontSize: { xs: '1.5rem', sm: '2.125rem' }, }}> Access Code Request </Typography>
          <Button disabled={!otpRequest[0] || otpRequest[0].access_status != 1} sx={{ background: otpRequest[0] && otpRequest[0].access_status == 1 ? '#16A22B' : '#ccc', color: 'white', fontWeight: 'bold', width: { xs: '100%', sm: '200px' }, height: '50px', borderRadius: '10px', marginTop: { xs: 1, sm: '2rem' }, '&:disabled': {backgroundColor: '#ccc',color: 'white',},}}onClick={() => handleOpenModal(otpRequest[0])}> Change Access Code</Button>
      </Box>
      {otpRequest.length > 0 ? (
          isSmallScreen ? (
            otpRequest.map((request) => (
              <Box key={request.id} sx={{ border: '1px solid #ccc', borderRadius: '5px', marginBottom: 2, padding: 2, boxShadow: 2 }}>
                ...
                <Typography><strong>Access Code:</strong> {request.access_status == 1 ? request.Access_code : '—'}</Typography>
                <Typography><strong>Status:</strong>
                <Typography  sx={{  color:request.access_status == 2 ? '#E87E21' : request.access_status == 1 ? '#16A22B' : request.access_status == 3 ? '#C41919' : 'inherit', fontWeight: 'bold', borderRadius: '5px', padding: '4px 12px', display: 'inline-block', textTransform: 'none', width: '100px', textAlign: 'center',}}>{request.access_status == 1 ? 'ACCEPTED' : request.access_status == 2 ? 'PENDING' : request.access_status == 3 ? 'REJECTED' : 'Unknown'}</Typography>
                </Typography>
                ...
                <TableBody>
                  {otpRequest.map((request) => (
                    <TableRow key={request.id}>
                      ...
                      <TableCell align="center">{request.access_status == 1 ? request.Access_code : '—'}</TableCell>
                      <TableCell align="center">
                        <Typography align="center" sx={{ color: request.access_status == 2 ? '#E87E21' : request.access_status == 1 ? '#16A22B' : request.access_status == 3 ? '#C41919' : 'inherit',
                          fontWeight: 'bold', borderRadius: '5px', padding: '4px 12px', display: 'inline-block', textAlign: 'center',
                        }}>
