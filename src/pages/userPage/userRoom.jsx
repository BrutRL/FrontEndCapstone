import { Box, Button, Typography, Dialog,Grid, DialogTitle, Input,InputAdornment,IconButton,DialogContent, DialogActions, TextField, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, MenuItem, Select } from '@mui/material'
...
function DisplayRoom() {
  ...
  const handleOtpAssign = async () => {
    if (userId && accessRoomDialog && usedAt && endTime && assignedDate && Purpose) {
      try {
        ...
        if (response.ok) {
          ...
          setErrors({}); 
          navigate('/user/access_code', { replace: true });
    ...
    if (userId && accessRoomDialog && usedAt && endTime && assignedDate && Purpose) {
      try {
        ...
        if (response.ok) {
          toast.success('Request successful!');
          setErrors({});
          navigate('/user/room_request', { replace: true });
        } else {
          if (response.errors || response.data?.errors) {
            setErrors(response.errors || response.data.errors);
          } else {
  ...
  return (
    <Box x sx={{ textAlign: isSmallScreen ? "center" : "left", mb: isSmallScreen ? 1 : 2 }}>
      <Typography variant="h4" gutterBottom sx={{ fontWeight: 'bold', color: '#374151', letterSpacing: '0.5px' }}> Rooms</Typography>
      ...
      <Dialog open={!!accessRoomDialog} onClose={() => setAccessRoomDialog(null)} maxWidth="md" fullWidth>
                              ...
                              <Box>
                                {filteredSchedules.map((schedule, index) => (
                                  <Box key={index} sx={{ p: 2, mb: 2, border: '1px solid #ccc', borderRadius: '8px', backgroundColor: index % 2 === 0 ? '#f9f9f9' : '#ffffff' }}>
                                    ...
                                    <Typography><b>Time:</b> {schedule.start_time.slice(0, 5)} - {schedule.end_time.slice(0, 5)}</Typography>
