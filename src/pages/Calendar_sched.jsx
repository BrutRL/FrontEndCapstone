...
function CalendarSched() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const formattedEvents = filteredData.flatMap(event => generateEvents(event));
  ...
  const CustomEvent = ({ event }) => {
    ...
    return (
      <div style={{ color: 'black', borderRadius: '8px', padding: '8px', fontSize: '0.85rem', lineHeight: '1.4', minHeight: '90px' }} >
        <strong>{event.course?.name || 'No code'}</strong>
        <div>Year: {event.year || 'Unknown'}</div>
        <div>Prof: {event.user?.username || 'Unknown'}</div>
  ...
  return (
    <Card sx={{ p: 2, boxShadow: 3, borderRadius: 3 }}>
      <Typography variant="h5" sx={{ mb: 2, color: '#374151', fontWeight: 'bold' }}>
        Calendar
      </Typography>
      <CardContent>
